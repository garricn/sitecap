  ...
  for (let i = 0; i < result.resources.length; i += BATCH_SIZE) {
    ...
    for (const resource of batch) {
      ...
      if (mediaTvNames.size > 0) {
        const hasMediaTv = Object.keys(resource.fields).some(name => mediaTvNames.has(name));
        // Also check if any media TV is missing from fields entirely
        const missingMediaTv = [...mediaTvNames].some(name => !(name in resource.fields));
        if (hasMediaTv || missingMediaTv) {
          try {
            ...
            if (editHtml) {
              // Collect all TV names (media + others) for parsing
              const allTvNames = new Set([...mediaTvNames, ...Object.keys(resource.fields)]);
