...
async function downloadFile(page, url) {
  const result = await page.evaluate(async ({ url: fetchUrl, timeout, maxSize }) => {
    ...
    try {
      ...
      // Reject oversized files before buffering when Content-Length is available
      const cl = r.headers.get("content-length");
      if (cl && parseInt(cl, 10) > maxSize) {
        throw new Error(`size_limit: ${cl} bytes exceeds ${maxSize}`);
      }
  ...
  }, { url, timeout: DOWNLOAD_TIMEOUT, maxSize: MAX_FILE_SIZE });
...
export async function downloadCmsMedia(page, cmsStructure, cmsDetection, outDir, opts = {}) {
  ...
  if (cms === "modx") {
    ...
    // Unsupported CMS — skip media download entirely
    return;
  }
