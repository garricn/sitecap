...
async function downloadFile(page, url) {
  const bytes = await page.evaluate(async ({ url: fetchUrl, timeout }) => {
...
function collectModxFileRefs(cmsStructure, sources) {
  ...
  for (const resource of (cmsStructure.resources || [])) {
    ...
    for (const [tvName, tvValue] of Object.entries(resource.fields)) {
      ...
      // Check if the TV value matches a media source baseUrl
...
async function downloadModxMedia(page, cmsStructure, cmsDetection, outDir, opts, manifest) {
  ...
  if (!siteId || !connectorsUrl) return;
  ...
  for (const ref of fileRefs) {
    ...
    try {
      ...
      const buf = await downloadFile(page, downloadUrl);

      if (buf.length > MAX_FILE_SIZE) {
        manifest.files.push({
          url: ref.url, localPath, mime: "", size: buf.length,
      ...
      manifest.files.push({
        url: ref.url, localPath, mime: "", size: 0,
        sourceId: ref.sourceId, referencedBy: ref.referencedBy, error: e.message,
      });
      manifest.stats.errors++;
    }
  }
