...
export async function extractWordPress(page, context) {
  ...
  try {
    ...
    // Context may be destroyed mid-crawl; don't crash extraction
  }
...
/**
 * Walk resolved ACF fields and collect media objects (those with id + url).
 * Returns a Map of id -> {id, url, mime, referencedBy: [...]}
 *
 * @param {object} cmsStructure - The extracted WordPress CMS structure
 * @returns {Map<number, {id: number, url: string, mime: string, referencedBy: string[]}>}
 */
export function collectWpMediaUrls(cmsStructure) {
  const mediaMap = new Map();

  function walk(obj, contentType, contentId, prefix) {
    if (obj == null || typeof obj !== "object") return;
    if (Array.isArray(obj)) {
      for (let i = 0; i < obj.length; i++) {
        walk(obj[i], contentType, contentId, `${prefix}.${i}`);
      }
      return;
    }
    // Check if this object is a resolved media object (has id + url)
    if (typeof obj.id === "number" && typeof obj.url === "string") {
      const ref = `${contentType}:${contentId}:${prefix}`;
      if (mediaMap.has(obj.id)) {
        mediaMap.get(obj.id).referencedBy.push(ref);
      } else {
        mediaMap.set(obj.id, {
          id: obj.id,
          url: obj.url,
      ...
      return; // Don't recurse into the media object itself
    }
    // Recurse into nested objects
    for (const [key, val] of Object.entries(obj)) {
      walk(val, contentType, contentId, prefix ? `${prefix}.${key}` : key);
    }
  }
  ...
  for (const post of (cmsStructure.posts || [])) {
    if (post.fields && typeof post.fields === "object") {
      ...
      for (const [key, val] of Object.entries(post.fields)) {
        walk(val, type, post.id, key);
      }
    }
  }
  ...
  for (const pg of (cmsStructure.pages || [])) {
    if (pg.fields && typeof pg.fields === "object") {
      for (const [key, val] of Object.entries(pg.fields)) {
        walk(val, "page", pg.id, key);
      }
    }
  }
...
async function resolveAcfMedia(page, apiRoot, nonce, items) {
  ...
  // 2. Batch-fetch each unique ID (batches of 10 to avoid hammering WP REST API)
  const mediaMap = new Map();
  const idArray = [...ids];
  const MEDIA_BATCH = 10;

  const results = {};
  for (let i = 0; i < idArray.length; i += MEDIA_BATCH) {
    const batch = idArray.slice(i, i + MEDIA_BATCH);
    const batchResults = await page.evaluate(async ({ apiRoot, nonce, batch }) => {
      const out = {};
  ...
  for (const item of items) {
    if (item.fields && typeof item.fields === "object") {
      item.fields = replaceIds(item.fields);
    }
  }
}
