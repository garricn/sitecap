...
class ExtensionLocator {
  ...
  _parseSelector() {
    ...
    if (nthMatch) return { sel: nthMatch[1], nth: parseInt(nthMatch[2], 10) };
    return { sel: this._selector, nth: this._nth };
  }

  /**
   * Build a JS expression that returns an array of matching elements.
   * Supports CSS selectors and Playwright's text= pseudo-selector.
   */
  static _queryJS(selector) {
    // text="exact match" — element whose trimmed textContent equals the string
    const textExact = selector.match(/^text="(.+)"$/);
    if (textExact) {
      const text = JSON.stringify(textExact[1]);
      // Find the smallest (deepest) element whose textContent matches exactly
      return `(function() { var matches = Array.from(document.querySelectorAll('*')).filter(function(el) { return el.textContent.trim() === ${text}; }); matches.sort(function(a, b) { return a.textContent.length - b.textContent.length; }); return matches; })()`;
    }
    // text=substring match — element whose textContent contains the string
    const textSub = selector.match(/^text=(.+)$/);
    if (textSub) {
      const text = JSON.stringify(textSub[1]);
      // Find elements containing text, prefer smallest (tightest match)
      return `(function() { var matches = Array.from(document.querySelectorAll('*')).filter(function(el) { if (!el.textContent.includes(${text})) return false; if (el.textContent.trim().length > 200) return false; return true; }); matches.sort(function(a, b) { return a.textContent.length - b.textContent.length; }); return matches; })()`;
    }
  ...
  async _resolve() {
    const { sel, nth } = this._parseSelector();
    const index = nth ?? 0;
    const queryJS = ExtensionLocator._queryJS(sel);
    const { result, exceptionDetails } = await this._page._cdp("Runtime.evaluate", {
      expression: `(function() { var els = ${queryJS}; return els[${index}] || null; })()`,
      returnByValue: false,
    });
  ...
  async all() {
    ...
    const queryJS = ExtensionLocator._queryJS(sel);
    const count = await this._page.evaluate(`(function() { return (${queryJS}).length; })()`);
    return Array.from({ length: count }, (_, i) => new ExtensionLocator(this._page, sel, i));
  }
  ...
  async waitFor(opts = {}) {
    ...
    const start = Date.now();
    while (Date.now() - start < timeout) {
      const visible = await this._page.evaluate(`(function() {
        var els = ${queryJS};
        var el = els[${index}];
        var wantState = ${JSON.stringify(state)};
        if (!el) return wantState === "hidden" || wantState === "detached";
        var rect = el.getBoundingClientRect();
        var hasSize = rect.width > 0 && rect.height > 0;
        var style = window.getComputedStyle(el);
        var isVisible = hasSize && style.display !== "none" && style.visibility !== "hidden";
        if (wantState === "hidden") return !isVisible;
