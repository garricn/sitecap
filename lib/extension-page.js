...
export class ExtensionPage {
  ...
  async goto(url) {
    ...
    // Skip navigation if this is an existing tab already at the URL
    if (this._existingTab && this._url === url) {
    ...
    this._existingTab = false; // no longer reusing after navigation
    return result;
  }
...
export async function createExtensionPage(bridge, opts = {}) {
  ...
  if (opts.url && opts.url !== "about:blank") {
    try {
      const result = await bridge.call("tabs.find", { url: opts.url });
      if (result.found) {
        tabId = result.tabId;
        existingTab = true;
      }
    } catch {
      // tabs.find not supported or failed, fall through to create
    }
  }

  // Create a new tab if no existing one found
  if (!tabId) {
  ...
  page._existingTab = existingTab;

  if (opts.viewport && !existingTab) {
