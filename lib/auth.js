...
async function executeSectionNode(node, page, context, opts, slugPrefix) {
      ...
      try {
        while (nextIdx < elements.length) {
          ...
          if (node.capture !== false) {
            ...
            if (node.clip === "element") {
              // Re-locate element fresh after click + settle (DOM may have mutated)
              captureOpts.screenshotLocator = tabPage.locator(`${node.selector} >> nth=${idx}`).first();
            }
  ...
  for (let idx = 0; idx < elements.length; idx++) {
    ...
    if (node.capture !== false) {
      ...
      if (node.clip === "element") {
        // Re-locate element fresh after click + settle (DOM may have mutated)
        captureOpts.screenshotLocator = page.locator(`${node.selector} >> nth=${idx}`).first();
      }
