 ...
 */
export function formatAXTree(nodes) {
  ...
  await page.waitForFunction(
    (qMs) => {
      return new Promise((resolve) => {
        ...
        // Track iframes we've already attached load listeners to
        const seenIframes = new WeakSet();

        function attachIframeListeners() {
          const iframes = document.querySelectorAll("iframe");
          for (const iframe of iframes) {
            if (seenIframes.has(iframe)) continue;
            seenIframes.add(iframe);
            if (!iframe.src || iframe.src === "about:blank") continue;
            // Reset on each iframe load event — cross-origin iframe loads
            // don't trigger MutationObserver on the parent document.
            iframe.addEventListener("load", () => reset(), { once: true });
            // Reset now if the iframe hasn't loaded yet
            try {
              if (!iframe.contentDocument || iframe.contentDocument.readyState !== "complete") {
                reset();
              }
            } catch {
              // Cross-origin — can't check readyState, reset to be safe
              reset();
            }
          }
        }
        ...
        const observer = new MutationObserver((mutations) => {
          reset();
          // Re-scan for dynamically added iframes (SPAs inject them after initial load)
          for (const mutation of mutations) {
            for (const node of mutation.addedNodes) {
              if (node.nodeName === "IFRAME" || (node.querySelectorAll && node.querySelectorAll("iframe").length)) {
        ...
        // Attach listeners to any iframes already in the DOM
        attachIframeListeners();
...
export async function capturePage(page, outDir, opts = {}) {
  ...
  // Screenshot (full page or element-clipped)
  if (types.has("screenshot")) {
    try {
      const filePath = join(outDir, "screenshot.png");
      if (opts.screenshotLocator) {
        // Element-level screenshot: try locator.screenshot(), fall back to boundingBox clip
        if (typeof opts.screenshotLocator.screenshot === "function") {
          await opts.screenshotLocator.screenshot({ path: filePath });
        } else {
          const box = await opts.screenshotLocator.boundingBox();
          if (box) {
  ...
  if (opts.downloadAssets) {
    try {
      ...
      const manifestData = shared
        ? { deduped: true, sharedDir: shared, assets: manifest }
        : manifest;
  ...
  if (types.has("cms")) {
    try {
      ...
      // Download CMS media files (opt-in)
      if (cmsStructure.extracted && opts.downloadMedia) {
        try {
          const { downloadCmsMedia } = await import("./cms/media.js");
          await downloadCmsMedia(page, cmsStructure, cmsData, outDir, opts);
          results.cmsMedia = join(outDir, "cms-media.json");
        } catch (e) {
          errors.cmsMedia = e.message;
        }

        // Generate coverage report if media was downloaded
        if (results.cmsMedia) {
          try {
            const { generateMediaCoverage } = await import("./cms/coverage.js");
            const cmsMediaManifest = JSON.parse(await readFileAsync(results.cmsMedia, "utf-8"));
            const coverage = generateMediaCoverage(cmsStructure, cmsMediaManifest);
            const coveragePath = join(outDir, "cms-media-coverage.json");
            await writeFile(coveragePath, JSON.stringify(coverage, null, 2));
            results.cmsCoverage = coveragePath;
          } catch (e) {
            errors.cmsCoverage = e.message;
          }
        }
      }
    } catch (e) {
 ...
 * @param {string} [opts.waitForText] - Wait for this text to appear in DOM before capturing.
 */
export async function navigateAndCapture(page, url, outDir, opts = {}) {
  ...
  // Optional pre-settle delay for iframe-heavy SPAs.
  // Runs before settle so iframes have time to appear in the DOM.
  // For SPAs that inject iframes asynchronously, --wait gives the app
  // time to render before settle's iframe detection kicks in.
  if (opts.waitMs && opts.waitMs > 0) {
    await new Promise((r) => setTimeout(r, opts.waitMs));
  }

  // Wait for specific text to appear in the DOM (for SPAs that render asynchronously)
  if (opts.waitForText) {
    const textTimeout = opts.maxTimeout || 10_000;
    await page.waitForFunction(
      (text) => document.body && document.body.innerText.includes(text),
      opts.waitForText,
      { timeout: textTimeout },
    ).catch(() => {
      // Text not found within timeout — proceed with capture anyway
    });
  }
