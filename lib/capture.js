  ...
  if (opts.downloadAssets) {
    try {
      ...
      for (const entry of (page.__sitecapNetwork || [])) {
        ...
        await writeFile(join(assetsDir, filename), entry.__body);
      ...
      results.assets = join(assetsDir, "manifest.json");
      ...
      if (results.html) {
        ...
        for (const [url, info] of Object.entries(manifest)) {
          // Replace full URL
          html = html.replaceAll(url, `assets/${info.file}`);
          // Also replace path-only form (e.g. /style.css from href="/style.css")
          try {
            const pathname = new URL(url).pathname;
            if (pathname && pathname !== "/") {
              html = html.replaceAll(`"${pathname}"`, `"assets/${info.file}"`);
              html = html.replaceAll(`'${pathname}'`, `'assets/${info.file}'`);
            }
