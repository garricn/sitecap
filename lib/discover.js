...
export async function discoverPatterns(page, opts = {}) {
  ...
  // Filter out noisy generic selectors
  return results.filter((r) => !isNoisySelector(r.selector));
}

/** Selectors that are too generic to be useful foreach targets. */
const NOISY_SELECTORS = /^(a|div|span|td|th|tr|li|p|text|svg|path|img|button)$/i;

function isNoisySelector(selector) {
  // Bare tag name with no class/id qualifier
  return NOISY_SELECTORS.test(selector.trim());
}
...
/**
 * Generate flat dot-path sections YAML from discovered patterns.
 *
 * @param {Array<object>} patterns - From discoverPatterns()
 * @param {object} opts
 * @param {string} opts.url - Source URL
 * @param {number} [opts.waitMs] - Wait time after click (default: 2000)
 * @returns {string} YAML content
 */
export function generateSectionsYAML(patterns, opts = {}) {
  const url = opts.url || "unknown";
  const waitMs = opts.waitMs || 2000;
  ...
  const lines = [
    "# Auto-generated by sitecap discover (sections format)",
    `# Source: ${url}`,
    "# Edit before running: sitecap <url> --explore <this-file> --extension -o ./output",
    "",
    `name: explore-${slugify(url)}`,
    "sections:",
  ];
  ...
  for (let i = 0; i < patterns.length; i++) {
    ...
    const sectionName = `pattern${i + 1}`;
    lines.push(`  # ${p.count} elements: "${p.selector}"`);
    if (p.sampleTexts.length > 0) {
      lines.push(`  # Sample: ${p.sampleTexts.slice(0, 3).join(", ")}`);
    }
