...
export async function createExtensionBridge(opts = {}) {
  ...
  /** Register message/close handlers on a WebSocket connection. */
  function registerClient(ws) {
    // Reject in-flight calls on the old socket before swapping
    if (client && client !== ws) {
      for (const [id, { reject: rej, timer: t }] of pending) {
        clearTimeout(t);
        rej(new Error("Extension reconnected — old calls dropped"));
        pending.delete(id);
      }
    }
    client = ws;
    ...
    ws.on("close", () => {
      // Only reject if this is still the active client
      if (client === ws) {
        client = null;
        for (const [id, { reject: rej, timer: t }] of pending) {
          clearTimeout(t);
  ...
  const connected = new Promise((resolve, reject) => {
    ...
    wss.on("connection", (ws) => {
      ...
      // Resolve on first hello
      const onFirstMsg = (data) => {
        try {
          const msg = JSON.parse(data);
          if (msg.type === "hello") resolve();
        } catch { /* ignore */ }
      };
      ws.once("message", onFirstMsg);
    });
  });

  // Handle reconnections (MV3 service worker restarts)
  wss.on("connection", (ws) => {
    // Skip if this is the initial connection (already handled above)
    if (ws === client) return;
    log("Extension reconnected (service worker restart)");
    registerClient(ws);
  });
