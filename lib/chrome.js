...
export async function launchChromeWithProfile(opts) {
  const { profileDir, userDataDir, viewport } = opts;
  ...
  // Clean up stale SingletonLock from force-killed Chrome
  try {
    await unlink(join(dataDir, "SingletonLock"));
  } catch {
    // No stale lock, or already cleaned up
  }

  // Use launchPersistentContext with channel:'chrome' to use the system Chrome
  // binary with full capabilities. This binds the debug port reliably on macOS.
  const context = await chromium.launchPersistentContext(profilePath, {
    ...
    args: [
      ...
      "--disable-features=RemoteDebuggingConfirmation",
    ],
  });
...
/**
 * Close a Playwright browser context (shuts down Chrome).
 * @param {import('playwright').BrowserContext} context
 */
export async function shutdownChrome(context) {
  if (context) {
    await context.close();
  }
}
