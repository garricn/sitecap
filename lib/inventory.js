...
export async function buildInventory(page, rootUrl, opts = {}) {
  ...
  async function inventoryPage(url, depth) {
    ...
    // Classify patterns: check if <a> with hrefs (navigation) or click-to-probe
    ...
    for (const pattern of patterns.slice(0, 5)) {
      ...
      // Fast path: if child elements are <a> tags with hrefs, classify as navigation
      if (pattern.childTag === "a") {
        try {
          const hrefs = await page.evaluate((sel) => {
            const els = document.querySelectorAll(sel);
            const currentPath = window.location.origin + window.location.pathname;
            return Array.from(els).map((el) => el.href).filter((h) => {
              if (!h || h.startsWith("javascript:")) return false;
              // Filter same-page anchors (href="#..." resolves to current page + #)
              try { const u = new URL(h); if (u.origin + u.pathname === currentPath && u.hash) return false; } catch { return false; }
              return true;
            });
          }, pattern.selector);
          // Require multiple links to classify as navigation (1 link is probably not a nav pattern)
          if (hrefs.length >= 3) {
            type = "navigation";
            children = [...new Set(hrefs)].slice(0, 10);
          }
        } catch { /* fallback to probe */ }
      }

      if (type === "panel" && pattern.clickable && pattern.count > 0) {
        try {
...
/**
 * Generate a flat dot-path sections YAML from an inventory.
 *
 * @param {object} inventory - From buildInventory()
 * @returns {string} YAML content
 */
export function generateInventorySectionsYAML(inventory) {
  const lines = [
    "# Auto-generated by sitecap inventory (sections format)",
    `# Root: ${inventory.root}`,
    `# Pages: ${inventory.summary.totalPages}, Patterns: ${inventory.summary.totalPatterns}`,
    `# Estimated: ${inventory.summary.estimatedCaptures} captures, ${inventory.summary.estimatedFiles} files`,
    "",
  ...
  for (const page of inventory.pages) {
    ...
    for (const pattern of page.patterns) {
      if (pattern.type === "panel" && pattern.score >= 10) {
        const sectionName = patternIdx === 0 ? pageSlug : `${pageSlug}.detail${patternIdx}`;
        lines.push(`  # ${pattern.count} elements: "${pattern.selector}"`);
    ...
    if (patternIdx === 0) {
      // No patterns — still emit a root section for the page capture
      lines.push(`  - name: ${pageSlug}`);
      lines.push(`    url: ${page.url}`);
      lines.push(`    selector: body`);
      lines.push("    wait: 2000");
      lines.push("    capture: false");
      lines.push("");
    }
